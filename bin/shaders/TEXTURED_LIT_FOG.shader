...
float positionResolution = 4096.0F;
float innacuracyOverDistanceFactor = 8.0F;
void main()  
{  
   gl_Position =  projectionMatrix * viewMatrix * modelMatrix * vec4(position, 1);
   //vertex jitter
   gl_Position.xy = floor(gl_Position.xy * (positionResolution / (gl_Position.w  * innacuracyOverDistanceFactor))) / (positionResolution / (clamp(gl_Position.w, -100.0F, 10000) * innacuracyOverDistanceFactor));
   vTexCoord = texCoord;
}  
